...
// these pages never pays for a live OPhim+TMDB build. worker/index.js's
...
// all 12 targets in one Worker invocation would mean up to 12 × (1 OPhim +
...
// single failed shard (e.g. OPhim 500s for one genre) just leaves THAT
...
// this same rule for the same OPhim-fetch-and-enrich shape. If that shape
  ...
  ...WARM_LIST_TYPES.map((type) => ({
    ...
    upstream:
      type === 'phim-moi-cap-nhat'
        ...
        : `${OPHIM_BASE}/v1/api/danh-sach/${type}?page=1`,
  })),
  ...WARM_GENRE_SLUGS.map((slug) => ({
    key: pageKey('/api/genre', [['slug', slug], ['page', '1']]),
    upstream: `${OPHIM_BASE}/v1/api/the-loai/${slug}?page=1`,
  })),
  ...WARM_COUNTRY_SLUGS.map((slug) => ({
    key: pageKey('/api/country', [['slug', slug], ['page', '1']]),
    upstream: `${OPHIM_BASE}/v1/api/quoc-gia/${slug}?page=1`,
  })),
];
