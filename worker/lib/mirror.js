...
// --- one-off GC: delete pre-migration key-shape objects (2026-08 domain
// migration, see state.md Phase 5) --------------------------------------
//
// Every reader now emits/expects the swapped `.webp` shape served from
// img.bluesia.net (worker/lib/images.js), so any `mirrored` row still in the
// old shape (`%.jpg.webp`, or a bare jpg-shaped key from before the WebP
// migration ever ran) is unreachable dead weight in the bucket. This route
// deletes the R2 object then its `mirrored` bookkeeping row, batched like the
// mirror drain above. TEMPORARY — remove this function + its route in
// worker/index.js once Phase 5 has fully drained (see state.md).
const GC_BATCH = 20;
const GC_OLD_SHAPE_WHERE = "key LIKE '%.jpg.webp' OR (key NOT LIKE '%.webp' AND key NOT LIKE 'ophim/%')";

async function gcOne(env, key) {
  try {
  ...
  try {
    await env.DB.prepare('DELETE FROM mirrored WHERE key = ?1').bind(key).run();
  } catch {
    return 'retry';
  }
  return 'deleted';
...
export async function drainGcBatch(env) {
  const { results } = await env.DB.prepare(
    `SELECT key FROM mirrored WHERE ${GC_OLD_SHAPE_WHERE} LIMIT ?1`
  )
    .bind(GC_BATCH)
    .all();
  const keys = (results || []).map((r) => r.key);
  if (!keys.length) return { drained: 0 };

  const counts = { deleted: 0, retry: 0 };
  await mapLimit(keys, MIRROR_CONCURRENCY, async (key) => {
    const outcome = await gcOne(env, key);
    counts[outcome] = (counts[outcome] || 0) + 1;
  });
  console.log('[gc drain]', { pulled: keys.length, ...counts });
  return { drained: keys.length, ...counts };
}
